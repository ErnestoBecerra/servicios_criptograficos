...
class practica0(minimal):
    ...
    def doEncrypt(self):
        ...
        # self.ds_filedata = self.ds_signature.to_bytes(cat.rsa.maxbytes, cat.cfg.endian)
    ...
    def doDecrypt(self):
        ...
        try:
            singsiz_str = self.ds_filedata.split(b'\n', 1)[0]
            signsiz = int(singsiz_str)
            begin_signature = len(singsiz_str)
            end_signature = begin_signature + signsiz

            self.ds_signature = int(self.ds_filedata[begin_signature:end_signature+1].decode(cat.cfg.txt_encode))
            self.ds_filedata = self.ds_filedata[end_signature+1:]

            print('\033[33m')
            print(f'signature size: {signsiz}\nsignature is: {self.ds_signature}')
            print('\033[0m')
